...
bool New3DScene::init() {
    ...
    auto texture = Director::getInstance()->getTextureCache()->addImage("Prechessboard.png");
    auto sprite = Sprite::createWithTexture(texture);
    sprite->setFlippedY(true); // Uncomment if you need to flip the texture

    _plane->setTexture("Prechessboard.png");
    _plane->addChild(sprite);
    scheduleUpdate(); // Schedule update method


   this->scheduleOnce(CC_SCHEDULE_SELECTOR(New3DScene::switchToNextScene), 8.0f);
...
void New3DScene::update(float delta) {
    if (_orc) {
        ...
        if (distance > 0.5f) { // Set a threshold to stop the movement
            ...
            _orc->setRotation3D(Vec3(0.0f, newAngle - 100.0f, 0.0f)); // Adjusting by -90 degrees
        }
#ifdef movement
        // 判断orc是否在移动
...
New3DScene::~New3DScene() {
    ...
    // 如果您保留了动作，需要在这里释放
    if (_orcWalkAction) {
        _orcWalkAction->release();
    }
    if (_orcIdleAction) {
        _orcIdleAction->release();
    }
#endif // movement
}
    // 注意：Cocos2d-x中的大多数对象（如Sprite3D、Camera等）会自动处理它们自己的内存，所以通常不需要在这里释放它们
...
//    // 手动释放3D资源
//    if (_orc) {
//        _orc->removeFromParentAndCleanup(true);
//        _orc = nullptr;
//    }
//    if (_plane) {
